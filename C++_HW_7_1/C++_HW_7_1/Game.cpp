...
bool GameDisplay(int mapnum, int enemynum, int mobnum1, int mobnum2, int mobnum3, int mobnum4, int mobnum5)
{
	...
	while (1)
	{
		...
		if (Creatures[attackmob]->GetHP() > 0)
		{
			...
			gotoxy(30, i);
			cout << Player->GetName() << "은(는) " << Creatures[attackmob]->GetName() << "에게 " << dmg << "의 피해를 주었다";
			i++;
			dmg = Creatures[attackmob]->Attack(Player);
			gotoxy(30, i);
			cout << Creatures[attackmob]->GetName() << "은(는) " << Player->GetName() << "에게 " << dmg << "의 피해를 주었다";
			i++;
		}	
		if (Creatures[attackmob]->GetHP() < 0)
		...
		getc = _getch();
		if (getc == 's' || getc == 'S' || getc == 'ㄴ')
		{
			system("cls");
			gotoxy(105, 10);
			cout << "게임을 종료하시겠습니까? ( Y / N ) : ";
			getc = getchar();
			if (getc == 'y' || getc == 'Y' || getc == 'ㅛ')
				return false;
			else if (getc == 'n' || getc == 'N' || getc == 'ㅜ')
				continue;
		}
